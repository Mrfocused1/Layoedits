...
app.get('/api/health', (req, res) => {
    res.json({
        status: 'ok',
        message: 'Layo Edits API is running',
        capabilities: {
            ffmpegAvailable,
            whisperAvailable,
    ...
    res.json({
        ...
        transcription: {
            ...
            recommendation: whisperAvailable
                ? 'Automatic transcription enabled - improves podcast editing quality'
...
app.post('/api/voice/upload', upload.single('audio'), async (req, res) => {
    ...
    try {
        ...
        formData.append('description', 'Podcast episode submitted via Layo Edits');
        ...
        // Add the voice audio file - this is the key field for audio processing
...
if (require.main === module) {
    app.listen(PORT, () => {
        console.log(`🚀 Layo Edits server running on http://localhost:${PORT}`);
